...
function Header() {
    ...
    return (
        <div className='w-full py-1 '>
            <div className={`fixed left-[37%] w-max h-max py-1 px-3 text-white text-sm font-bold tracking-[1px] hidden lg:flex justify-center items-center
                 gap-2 rounded-lg bg-gray-700 ${order ? " top-22 visible opacity-100" : "top-20 invisible opacity-0"} transition-all duration-300 ease-in-out shadow shadow-black `}> <CircleCheck size={18} className='text-green-500' />Succsessfully Added</div>

            <div className={` ${order ? " bottom-8 visible opacity-100" : "bottom-5 invisible opacity-0"} flex xl:hidden justify-center items-center transition-all duration-300 ease-in-out w-full h-max fixed `}>
                <div className={` left-[37%] w-max h-max py-1 px-3 text-white text-sm font-bold tracking-[1px] flex lg:hidden justify-center items-center
                 gap-2 rounded-lg bg-gray-700  shadow shadow-black `}> <CircleCheck size={18} className='text-green-500' />Succsessfully Added</div>
            </div>
            <div className=" hidden sm:block lg:hidden mt-15 text-xl text-center tt tracking-[0.2em] text-black select-none  uppercase">
                Designed with ux <span className="text-red-600 rt font-bold">by&nbsp;|&nbsp;ARHAM</span>
            </div>
            ...
            <div className={`w-[80%] sm:w-[80%] cont lg:w-[650px] xl:w-[700px] h-[max] lg:h-[580px] xl:h-[610px] mt-5 sm:mt-10 lg:mt-0 flex flex-col gap-5 xl:gap-7 justify-center pl-4 sm:pl-10 transition-all duration-150 ease-linear relative  `} >
                ...
                <button className={`py-2 px-4 w-max bg-black rounded-xl text-white text-sm sm:text-xl border border-transparent transition-all duration-200 ease-in hover:bg-white hover:border-black hover:text-black active:scale-95 cursor-pointer
                     ${anime2 ? "opacity-100 translate-y-0 " : "opacity-0 translate-y-10"} `} onClick={() => { dispatch(Add_to_cart(opt)); setorder(true) }} >Order Now</button>
            ...
            <div className="w-[40px] scale-130 mx-auto sm:hidden block mt-7 chef cursor-context-menu "><img className='drop-shadow-sm' src={logo} alt="" /></div>

            <div className="block sm:hidden mt-10 text-sm text-center sig tt tracking-[0.2em] text-black select-none  uppercase">
                Designed with ux <span className="text-red-600 rt underline font-bold">by&nbsp;|&nbsp;ARHAM</span>
            </div>
        </div>
    )
}
